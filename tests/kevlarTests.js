...
tests.unit.add( new Ext.test.TestSuite( {
	...
	items : [
		 ...
		 */
		{
			...
			"sync() should call the 'success' and 'complete' callbacks when no persistence operations need to be done" : function() {
				var models = this.createModels( 4 );
				var collection = new Kevlar.Collection( models );
			...
			"sync() should return a jQuery.Promise object which has its `done` and `always` callbacks executed when no models need to be persisted" : function() {
				var models = this.createModels( 4 );				
				var collection = new Kevlar.Collection( models );
